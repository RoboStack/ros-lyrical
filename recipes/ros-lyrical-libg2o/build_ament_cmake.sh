...
if [ "${PKG_NAME}" == "ros-lyrical-rmw-wasm-cpp" ]; then
  WORK_DIR=$SRC_DIR/$PKG_NAME/src/work/rmw_wasm_cpp
elif [ "${PKG_NAME}" == "ros-lyrical-wasm-cpp" ]; then
  WORK_DIR=$SRC_DIR/$PKG_NAME/src/work/wasm_cpp
elif [ "${PKG_NAME}" == "ros-lyrical-dynmsg" ]; then
...
$CMAKE_GEN \
    ...
    $EXTRA_CMAKE_ARGS \
    -DG2O_BUILD_APPS=OFF -DG2O_BUILD_EXAMPLES=OFF -DG2O_USE_OPENGL=OFF -DBUILD_LGPL_SHARED_LIBS=ON \
    $WORK_DIR
